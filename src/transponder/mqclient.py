...
class MQClient(object):
    ...
    def on_open_error(self, connection, reason):
        reconnect_in = 5
        logging.warning('Attempting reconnect in %ds' % reconnect_in)
        tornado.ioloop.IOLoop.instance().call_later(reconnect_in, self.reconnect)
        # self.reconnect()
    ...
    def setup_queue(self, queue_name):
        ...
        self._channel.queue_declare(
            queue_name,
            exclusive=True,    # only allow one consumer
            auto_delete=True,  # delete when consumer cancels or disconnects
    ...
    def queue_bind(self, queue_name, exchange_name, routing_keys, *args):
    ...
    def start_consuming(self):
        ...
        self._consumer_tag = self._channel.basic_consume(self._queue_name,
                                                         self.on_message)
...
if __name__ == "__main__":
        ...
        client = MQClient(
            mquri, mqexcname, mqexctype, routing_key=mqrk,
            handler=handler
        )
        client.run()
